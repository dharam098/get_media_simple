...
retries = Retry(total=5, backoff_factor=1, status_forcelist=[ 502, 503, 504 ])
s.mount('http://', HTTPAdapter(max_retries=retries))
quality_map = {'4K': 1, '1080p': 2, '720p': 3, 'SD': 4, 'CAM': 5}
tmdb_api_key = 'bbaa8919f1f6d5274a6835d71e37d20b'
debrid_api = '257Z637RY5VU2XLAEWM2ICYXQHS6YG4C3I7PIGW6W6C7TMXSBBMQ'
torrents = py1337x(proxy='1337x.to', cache='py1337xCache', cacheTime=500)
...
def suggest_movie_names(string):
    ...
    # Get a list of dictionaries containing movie/TV show titles and release years
    movie_info = [{'type': 'movie', 'id': movie['id'], 'title': '🎬 '+ movie['title'] , 'year': movie['release_date'].split('-')[0], 'popularity': movie['popularity']} for movie in movie_data['results']]
    tv_info = [{'type': 'tv', 'id': show['id'], 'title': '📺 '+ show['name'] , 'year': show['first_air_date'].split('-')[0], 'popularity': show['popularity']} for show in tv_data['results']]
    ...
    results.drop_duplicates(subset = 'title', inplace=True)
    # Find close matches between the input string and the movie/TV show titles
    suggested_names = [(string, (string, ' ', 0))]
    for i, info in results.iterrows():
        suggested_names.append((info['title'], (info['title'], info['type'], info['id'])))
        suggested_names.append((info['title'] + ' '+ info['year'], (info['title'] + ' '+ info['year'], info['type'], info['id'])))
    
    return suggested_names


def search_tmdb(searchterm: str) -> list[tuple[str, tuple]]:
    return suggest_movie_names(searchterm) if searchterm else [(' ', (' ', ' ', 0))]


## get season list
def season_list_tmdb(id):
    ...
    return [-1]+ df['season_number'].to_list()

## get episode list
def episode_list_tmdb(series_id, season_number):
    if season_number == -1:
        return [-1]
    ...
    headers = {"accept": "application/json"}
    response = requests.get(url, headers=headers, params = params)  
    df = pd.DataFrame(response.json()['episodes'])  
    return [-1]+df['episode_number'].to_list()
    


## generate query list
def get_query_list(query):
    category, title ,season , episode  = query['category'], query['title'], query['season'], query['episode']
    query_list = []
    if (category == 'tv') & (season != -1):
        if episode != -1:
            query_list.append(f"{title} s{str(season).zfill(2)} e{str(episode).zfill(2)}")
        query_list.append(f"{title} s{str(season).zfill(2)}")
        query_list.append(f"{title} season {season}")
        query_list.append(title)
        if episode != -1:
            query_list.append(f"{title} season {season} episode {episode}")
...
def search(query, site , page=1, category ='All', sortBy = 'seeders'):
    df= pd.DataFrame()
    if site.lower() == 'thepiratebay':
        df = search_tpb(query, category)

    elif site.lower() == '1337x':
        df = search_1337x(query, page, category = category, sortBy = sortBy)
        
    elif site.lower() == 'animetosho':
        df = search_anime_tosho(query)
...
def search_tpb(query, category = 'all'):
    ...
    if response.status_code == 200:
        ...
        if results[0]['name'] == 'No results returned':
            results = {'info_hash': []}
        df = pd.DataFrame(results)
        df.rename({'info_hash': 'infoHash'}, axis=1, inplace=True)
        return df
...
def search_1337x(query, page, category, sortBy):
    # get torrent list by search
    try:
        if(category=='All'):
            results_dic = torrents.search(query, page = page , sortBy = sortBy)
        else:
            results_dic = torrents.search(query, page = page , category=category, sortBy = sortBy)
...
def extract_season_and_episode(title: str):
    season = -1
    episode = -1
    
    # Search for season number
    season_match = re.search(r'(?i)(?<=\s)(?:season|s)\s*(\d+)', title)
    if season_match:
        season = int(season_match.group(1))
        season_start_index, season_end_index = season_match.span()
        title = title[:season_end_index]  + ' ' + title[season_end_index:]
        
    
    # Search for episode number
    episode_match = re.search(r'(?i)(?<=\s)(?:episodes|episode|ep|e)\s*(\d+)', title)
    if episode_match:
...
def rearrange_results(df, query):

    df.loc[:, 'clean title'] = df['name'].apply(lambda x: clean_title(x))
    df.loc[:, 'quality'] = df['clean title'].apply(lambda x: get_quality(x))
    df.loc[:, 'quality_'] = df['quality'].map(quality_map)
    df['seeders'] = df['seeders'].astype(int)
    df.sort_values(by = ['quality_', 'seeders'],ascending = [True, False], inplace=True)
    df.reset_index(drop=True, inplace=True)

    if query['category'] == 'tv':
        df['season'] = -1
        df['episode'] = -1
        for i , row in df.iterrows():
            title = df.loc[i, 'clean title']
            s, ep = extract_season_and_episode(title)
            df.loc[i, 'season'] =s
            df.loc[i, 'episode'] = ep
...
## filter relavant results
def filter_relevant_results(df, query):
    name_match = df['clean title'].str.contains(query['title'])
    season_match = pd.Series([True]*len(df))
    episode_match = season_match
    if query['category'] == 'tv':
        season_match = df['season']==query['season']
        episode_match = (df['episode'] == query['episode']) | (df['episode'] == -1)

    df_filtered = df[name_match & season_match & episode_match]
    df_residue = df.drop(df_filtered.index)
    return df_filtered, df_residue

## main
 
def get_results(query,site, page, sortby):
    df= df_filtered= df_residue = pd.DataFrame()

    query_list = get_query_list(query)
    
    for query_string in query_list:
        st.write(query_string)
        df = search(query_string, site, page, query['category'], sortby)
        if len(df):
            break
    
    
    if site.lower() == 'animetosho':
        df_filtered, df_residue = df, pd.DataFrame()
    else:
        df = rearrange_results(df, query)
        df_filtered, df_residue = filter_relevant_results(df, query)
    
    if site.lower() == '1337x':
        df_filtered = get_info_hash_1337x(df_filtered)

        
    return df, df_filtered, df_residue
...
def filter_cached_results(df):
    ...
    hash_list = '/'.join(df.loc[:,'infoHash'].to_list())
    url = f"https://api.real-debrid.com/rest/1.0/torrents/instantAvailability/{hash_list}"
    response = requests.get(url,headers={"Authorization": f"Bearer {debrid_api}"})
    df['cache_info'] = df.apply(lambda row: file_list(response.json()[row.infoHash.lower()]), axis = 1)
    df['number_of_files'] = df.apply(lambda row: len(row.cache_info), axis = 1)
    df=df[df['number_of_files']>0].reset_index(drop=True)
    st.session_state['df_cached'] = df
...
def get_debrid_link(df_cached, i):
    df_files = pd.DataFrame()
    info_hash = df_cached.loc[i,'infoHash']
    magnet = df_cached.loc[i,'magnet']

    # check link in cloud
    try:
        found, df_files, links = cloud_scanning(info_hash)
        if found:
            return df_files, links
        id = add_magnet_to_debrid(magnet)
        start_magnet(id) 
        df_files, links = get_cached_magnet_files(id)
    except:
        st.write('error: debrid failure!')

    return df_files, links

### cloud scanning
def cloud_scanning(info_hash):

    st.write('Cloud scanning')
    response = requests.get(
        'https://api.real-debrid.com/rest/1.0/torrents'
        ,headers={"Authorization": f"Bearer {debrid_api}"}
    )
    df_cloud = pd.DataFrame(response.json())
    matching_indexes = df_cloud.index[df_cloud['hash'] == info_hash].to_list()
    if len(matching_indexes):
...
###get cached magnet link
def get_cached_magnet_files(id):
...
def unrestrict(links, i=[-1]):
    ...
    for j,link in enumerate(links):
        ...
        try:
            ...
            response = response.json()
            download_links.append((response['filename'],response['download']))
...
def vlc_playlist(download_links):
    s = '#EXTM3U\n'
    df = pd.DataFrame(download_links).sort_values(by=0).reset_index(drop=True)
    for index, row in df.iterrows():
        s=s+'#EXTINF:-1,'+ row[0]+'\n'+row[1]+'\n'
...
def show_scrape_results(query_dic):
    site = st.session_state.get('selected_site', 'thepiratebay')
    sortBy = 'seeders'
    df_torrents, df_filtered, df_residue = get_results(query_dic, site, 1, sortBy)

    if len(df_torrents)==0:
        st.write('No Results!')
        return None  
    df_cached = filter_cached_results(df_filtered)
    st.session_state['df_cached'] = df_cached
    # st.session_state['df_cached'] = df_cached
    number_of_results = len(df_cached)
    ...
    for i in range(0, number_of_results):
        ...
        with st.session_state[f'container{i}']:
            ...
            st.session_state[f"container{i}button"] = st.button("link", key = f"container{i}button_")
            if st.session_state[f"container{i}button_"]:   
                df_files, links = get_debrid_link(df_cached, i)
                debrid_result = unrestrict(links)
                link, streamable  = st.columns((3,2))
                with link:
                    for file in debrid_result:
                        name1 = file[0]
...
if 'selected_sorting_method' not in st.session_state:
    st.session_state['selected_sorting_method'] = 'seeders'
...
# st.session_state['selected_sorting_method'] = st.sidebar.selectbox('sortby', ['seeders', 'time'])
search_bar, season, episode = st.columns((4,1,1))
with search_bar:
    st.session_state['query'] = st_searchbox(search_tmdb,key="search..", label="Title")
...
if query_global is None:
    ...
    if query_global[1]=='movie':
        query['title'] = clean_title(query_global[0])
        query['category'] = 'movie'
        query['season'] = -1
        query['episode'] = -1
    elif query_global[1]=='tv':
        query['title'] = clean_title(query_global[0])
        query['category'] = 'tv'
        with season:
            query['season'] = st.selectbox('Season', season_list_tmdb(query_global[2]))
        with episode:
            query['episode'] = st.selectbox('Episode', episode_list_tmdb(query_global[2],query['season']))
    else:
        query['title'] = clean_title(query_global[0])
        query['category'] = 'all'
        query['season'] = -1
    ...
    st.session_state['submit_clicked'] = st.button('Submit')
